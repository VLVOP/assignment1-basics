...
def run_linear(
    ...
    model = llmLinearModel(d_in, d_out, device=weights.device, dtype=weights.dtype)
...
def run_embedding(
    ...
    """
    model = llmEmbeddingModel(vocab_size, d_model, device=weights.device, dtype=weights.dtype)
    model.load_state_dict({'matrix': weights})
